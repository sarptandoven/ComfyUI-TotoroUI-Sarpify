import totoro.utils
import totoro_extras.nodes_post_processing
import torch

def reshape_latent_to(target_shape, latent):
    if latent.shape[1:] != target_shape[1:]:
        latent = totoro.utils.common_upscale(latent, target_shape[3], target_shape[2], "bilinear", "center")
    return totoro.utils.repeat_to_batch_size(latent, target_shape[0])
...
class LatentBatch:
    ...
    def batch(self, samples1, samples2):
        ...
        if s1.shape[1:] != s2.shape[1:]:
            s2 = totoro.utils.common_upscale(s2, s1.shape[3], s1.shape[2], "bilinear", "center")
...
class LatentOperationSharpen:
    ...
    def op(self, sharpen_radius, sigma, alpha):
        def sharpen(latent, **kwargs):
            ...
            kernel = totoro_extras.nodes_post_processing.gaussian_kernel(kernel_size, sigma, device=luminance.device)
